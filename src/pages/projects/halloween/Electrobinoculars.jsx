// src/pages/projects/misc/Electrobinoculars.jsx
...
// photos in the folder — see public/photos/electrobinoculars/README.md.
const DESCRIPTION = describe('electrobinoculars')

// Every photo in public/photos/electrobinoculars/, in filename order. The folder is
// read at build time (see scripts/photo-manifest.mjs), so adding a photo is a
// matter of dropping the file in.
const PHOTOS = (pagePhotos['electrobinoculars'] ?? []).map(asset)

export default function Electrobinoculars() {
  ...
  return (
    <div style={{ background: 'var(--bg)', color: 'var(--text)', fontFamily: 'system-ui' }}>
      ...
      {PHOTOS.length > 0 && (
        ...
        >
          <PhotoGroup photos={PHOTOS} heading="Electrobinoculars" />
        </div>
      )}
    </div>
  )
