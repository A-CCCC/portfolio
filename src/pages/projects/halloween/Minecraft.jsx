// src/pages/projects/misc/Minecraft.jsx
...
// photos in the folder — see public/photos/minecraft/README.md.
const DESCRIPTION = describe('minecraft')

// Every photo in public/photos/minecraft/, in filename order. The folder is
// read at build time (see scripts/photo-manifest.mjs), so adding a photo is a
// matter of dropping the file in.
const PHOTOS = (pagePhotos['minecraft'] ?? []).map(asset)
  ...
  return (
    <div style={{ background: 'var(--bg)', color: 'var(--text)', fontFamily: 'system-ui' }}>
      ...
      {PHOTOS.length > 0 && (
        ...
        >
          <PhotoGroup photos={PHOTOS} heading="Minecraft" />
        </div>
      )}
    </div>
  )
